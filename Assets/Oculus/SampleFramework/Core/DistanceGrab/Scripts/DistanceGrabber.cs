    ...
    public class DistanceGrabber : OVRGrabber
    {
        ...
        protected override void GrabBegin()
        {
            ...
            if (closestGrabbable != null)
            {
                ...
                //if (closestGrabbable.SnapToOrigin)
                //{
                //    m_startPos = m_grabbedObj.grabbedRigidbody.transform.position;
                //    m_startRot = m_grabbedObj.grabbedRigidbody.transform.rotation;
                //    startTransform = closestGrabbable.gameObject.transform;
                //}
        ...
        private bool grabJustStarted = false;
        protected override void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
        {
            if (m_grabbedObj == null)
            {
                grabJustStarted = false;
                return;
            }
            ...
            if (!grabJustStarted)
            {
                grabJustStarted = true;

                if (m_grabbedObj.SnapToOrigin)
                {
                    m_startPos = m_grabbedObj.grabbedRigidbody.transform.position;
                    m_startRot = m_grabbedObj.grabbedRigidbody.transform.rotation;
                }
            ...
            if (requestGrabRelease)
            {
                grabbedRigidbody.MovePosition(m_startPos);
                grabbedRigidbody.MoveRotation(m_startRot);
                
                if (Vector3.Distance(grabbedRigidbody.transform.position, m_startPos) < 0.001f)
                {
                    requestGrabRelease = false;
                    m_grabbedObjReset(Vector3.zero, Vector3.zero);
                }

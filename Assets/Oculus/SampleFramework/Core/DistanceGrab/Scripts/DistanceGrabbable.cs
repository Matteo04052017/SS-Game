        ...
        void RefreshCrosshair()
        {
            ...
            if (m_materialColorField != null)
            {
                ...
                if (isGrabbed || !InRange)
                    m_mpb.SetColor(m_materialColorField, Color.clear); // teo 31/01/2019
                else
                    if (Targeted)
                        // teo funziona solo con il proprio shader, quello del sole non va bene: deve essere unlit!
                        // considerare altre possibilità
                        m_mpb.SetColor(m_materialColorField, Color.green);
                    else
                        m_mpb.SetColor(m_materialColorField, Color.clear);// m_crosshairManager.OutlineColorInRange); // teo 31/01/2019

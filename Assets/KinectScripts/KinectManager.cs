...
public class KinectManager : MonoBehaviour
{
    ...
    private float LastObjectCreatedTime = 0;
	void Update()
	{
		if(KinectInitialized)
		{
			...
			if(Player1Calibrated)
			{
				...
				foreach(KinectGestures.GestureData gestureData in player1Gestures)
				{
					...
					{
						if(gestureData.gesture == KinectGestures.Gestures.RaiseRightHand)
						{
                            // teo instantiate object if click
                            if (InstantiateOnClickObject != null && (Time.time - LastObjectCreatedTime > 3))
                            {
                                GameObject obj = Instantiate(InstantiateOnClickObject, gestureData.screenPos, Quaternion.identity);
                                LastObjectCreatedTime = Time.time;
...
//					}
	...
	void RemoveUser(uint UserId)
	{
		...
		AllPlayersCalibrated = allUsers.Count >= 1; // false;
